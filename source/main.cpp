...
int main(int argc, char const* argv[]) {
  srand(time(NULL));
  ray::Material mat({1.0, 1.0, 1.0}, 30.0, 1.0, 1.0);
  ray::Material refl({1.0, 1.0, 1.0}, 30.0, 1.0, 0.8);
  std::vector<std::unique_ptr<ray::Object>> objs;
  std::vector<std::unique_ptr<ray::Light>> lights;
  lights.push_back(ray::MakePointLight({-5.0, 5.0, 20.0}, {1.0, 1.0, 1.0}));
  lights.push_back(ray::MakePointLight({5.0, 5.0, 20.0}, {1.0, 1.0, 1.0}));
